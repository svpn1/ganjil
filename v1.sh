...
data_ip="https://raw.githubusercontent.com/svpn1/ganjil/main/ip"
checking_sc() {
...
# USERNAME #
rm -f /usr/bin/user
username=$(curl https://raw.githubusercontent.com/svpn1/ganjil/main/ip | grep $MYIP | awk '{print $2}')
echo "$username" >/usr/bin/user
expx=$(curl https://raw.githubusercontent.com/svpn1/ganjil/main/ip | grep $MYIP | awk '{print $3}')
echo "$expx" >/usr/bin/e
# DETAIL ORDER #
...
Exp1=$(curl https://raw.githubusercontent.com/svpn1/ganjil/main/ip | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
...
print_install "Membuat direktori xray"
    ...
    mkdir -p /var/lib/jbr >/dev/null 2>&1
    # // Ram Information
...
echo "IP=" >> /var/lib/jbr/ipvps.conf
echo $host1 > /etc/xray/domain
echo $host1 > /root/domain
echo ""
elif [[ $host == "2" ]]; then
#install cf
wget ${REPO}project3/cf.sh && chmod +x cf.sh && ./cf.sh
rm -f /root/cf.sh
clear
else
...
restart_system() {
    USRSC=$(wget -qO- https://raw.githubusercontent.com/svpn1/ganjil/main/ip | grep $ipsaya | awk '{print $2}')
    EXPSC=$(wget -qO- https://raw.githubusercontent.com/svpn1/ganjil/main/ip | grep $ipsaya | awk '{print $3}')
    TIMEZONE=$(printf '%(%H:%M:%S)T')
    TEXT="
<code>────────────────────</code>
<b>⚡AUTOSCRIPT PREMIUM V.1 LTS⚡</b>
<code>────────────────────</code>
...
rm -rf /etc/vmess/.vmess.db
    ...
    mkdir -p /etc/jbr/limit/vmess/ip
    mkdir -p /etc/jbr/limit/vless/ip
    mkdir -p /etc/jbr/limit/trojan/ip
    mkdir -p /etc/jbr/limit/ssh/ip
...
clear
    ...
    # / / Xray Core Version Terbaru
latest_version="$(curl -s https://api.github.com/repos/XTLS/Xray-core/releases | grep tag_name | sed -E 's/.*"v(.*)".*/\1/' | head -n 1)"
bash -c "$(curl -L https://github.com/XTLS/Xray-install/raw/main/install-release.sh)" @ install -u www-data --version $latest_version
 
    # // Config Server
    wget -O /etc/xray/config.json "${REPO}project6/config.json" >/dev/null 2>&1
    wget -O /etc/systemd/system/runn.service "${REPO}project6/runn.service" >/dev/null 2>&1
    domain=$(cat /etc/xray/domain)
    ...
    wget -O /etc/haproxy/haproxy.cfg "${REPO}project6/haproxy.cfg" >/dev/null 2>&1
    wget -O /etc/nginx/conf.d/xray.conf "${REPO}project6/xray.conf" >/dev/null 2>&1
    sed -i "s/xxx/${domain}/g" /etc/haproxy/haproxy.cfg
    sed -i "s/xxx/${domain}/g" /etc/nginx/conf.d/xray.conf
    curl ${REPO}project6/nginx.conf > /etc/nginx/nginx.conf
...
print_install "Setting SSH Password"
    wget -O /etc/pam.d/common-password "${REPO}project2/password"
...
wget -O ${REPO}project8/limit.sh && chmod +x limit.sh && ./limit.sh
cd
wget -q -O /usr/local/sbin/quota "${REPO}project8/quota"
chmod +x /usr/local/sbin/quota
cd /usr/local/sbin/
sed -i 's/\r//' quota
cd
wget -q -O /usr/bin/limit-ip "${REPO}project8/limit-ip"
chmod +x /usr/bin/*
cd /usr/bin
sed -i 's/\r//' limit-ip
clear
wget -q -O /usr/bin/limit-ip-ssh "${REPO}project8/limit-ip-ssh"
chmod +x /usr/bin/*
cd /usr/bin
...
# // Installing UDPGW
wget -O /usr/bin/badvpn "${REPO}project9/badvpn" >/dev/null 2>&1
chmod +x /usr/bin/badvpn > /dev/null 2>&1
wget -q -O /etc/systemd/system/badvpn1.service "${REPO}project9/badvpn1.service" >/dev/null 2>&1
wget -q -O /etc/systemd/system/badvpn2.service "${REPO}project9/badvpn2.service" >/dev/null 2>&1
wget -q -O /etc/systemd/system/badvpn3.service "${REPO}project9/badvpn3.service" >/dev/null 2>&1
...
print_install "Installing the SlowDNS Server module"
    wget -q -O /tmp/nameserver "${REPO}project7/nameserver" >/dev/null 2>&1
...
wget -q -O /etc/ssh/sshd_config "${REPO}project2/sshd" >/dev/null 2>&1
...
wget -q -O /etc/default/dropbear "${REPO}project2/dropbear.conf"
...
#OpenVPN
wget ${REPO}project2/openvpn &&  chmod +x openvpn && ./openvpn
...
printf "q\n" | rclone config
wget -O /root/.config/rclone/rclone.conf "${REPO}project1/rclone.conf"
...
wget -q -O /etc/ipserver "${REPO}project1/ipserver" && bash /etc/ipserver
...
gotop_latest="$(curl -s https://api.github.com/repos/xxxserxxx/gotop/releases | grep tag_name | sed -E 's/.*"v(.*)".*/\1/' | head -n 1)"
    ...
    wget ${REPO}project3/bbr.sh &&  chmod +x bbr.sh && ./bbr.sh
...
# Ganti Banner
wget -O /etc/julak.txt "${REPO}project1/julak.txt"
...
print_install "Installing WebSocket Proxy"
    wget -O /usr/bin/ws "${REPO}project4/ws" >/dev/null 2>&1
    wget -O /usr/bin/tun.conf "${REPO}project4/tun.conf" >/dev/null 2>&1
    wget -O /etc/systemd/system/ws.service "${REPO}project4/ws.service" >/dev/null 2>&1
...
wget -O /usr/sbin/ftvpn "${REPO}project4/ftvpn" >/dev/null 2>&1
...
clear
    ...
    wget -q ${REPO}project0/udp-custom.sh &&  chmod +x udp-custom.sh && ./udp-custom.sh
}
...
echo -e "\033[0;32m │\033[0m            \033[0;36mPORT JULAK BANTUR AUTOSCRIPT\033[0m             \033[0;32m|\033[0m"
echo -e "\033[0;32m └──────────────────────────────────────────┘\033[0m"
echo -e "\033[0;32m┌─────────────────────────────────────────────┐"
...
echo -e "\033[0;32m│   - Trojan WS               : 443, 80           │"
